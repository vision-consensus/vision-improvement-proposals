```
vip: 721
title: VRC-721 Non-Fungible Token Standard
author: Darylc
status: Final
...
This VIP is compatible with [EIP-721](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-721.md)
...
- NFT also plays a key role in projects like vBTC, to prove the ownership of staking.

NFT tokens are added more value by those different applications. And more and more DeFi projects try to integrate with NFTs to enhance their ecso-system. We believe that standardized NFTs will open an infinitely imaginary space on Vision as well.

## Motivation

A standard interface allows applications to track and transfer NFTs on Vision. Simple VRC-721 smart contracts list as below to track a large number of NFTs. VRC-20 token standard is insufficient for handling NFTs due to each token in VRC-721 being unique. The standard of VRC-721 is inspiring on Vision. It plays an important role as well as VRC-20.

## Specification

The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in RFC 2119.

**Every VRC-721 compliant contract must implement the `VRC721` and `VRC165` interfaces** (subject to "caveats" below):
...
///  Note: the VRC-165 identifier for this interface is 0x80ac58cd.
interface VRC721 /* is VRC165 */ {
    ...
    ///  `onVRC721Received` on `_to` and throws if the return value is not
    ///  `bytes4(keccak256("onVRC721Received(address,address,uint256,bytes)"))`.
...
interface VRC165 {
    ...
    /// @param interfaceID The interface identifier, as specified in VRC-165
    /// @dev Interface identification is specified in VRC-165. This function
...
```solidity
/// @dev Note: the VRC-165 identifier for this interface is 0x5175f878.
interface VRC721TokenReceiver {
    /// @notice Handle the receipt of an NFT
    /// @dev The VRC721 smart contract calls this function on the recipient
    ...
    /// @return `bytes4(keccak256("onVRC721Received(address,address,uint256,bytes)"))`
    ///  unless throwing
    function onVRC721Received(address _operator, address _from, uint256 _tokenId, bytes _data) external returns(bytes4);
}
```
**Note:The hash of `bytes4(keccak256("onVRC721Received(address,address,uint256,bytes)))` is different from the Ethereum version `bytes4(keccak256("onERC721Received(address,address,uint256,bytes)))`. Please use `0x47f3171e` instead of `0x150b7a02`.**

The **metadata extension** is OPTIONAL for VRC-721 smart contracts (see "caveats", below). This allows your smart contract to be interrogated for its name and for details about the assets which your NFTs represent.

```solidity
/// @title VRC-721 Non-Fungible Token Standard, optional metadata extension
/// @dev See https://eips.ethereum.org/EIPS/eip-721
///  Note: the VRC-165 identifier for this interface is 0x5b5e139f.
interface VRC721Metadata /* is VRC721 */ {
    ...
    ///  3986. The URI may point to a JSON file that conforms to the "VRC721
...
The **enumeration extension** is OPTIONAL for VRC-721 smart contracts (see "caveats", below). This allows your contract to publish its full list of NFTs and make them discoverable.

```solidity
/// @title VRC-721 Non-Fungible Token Standard, optional enumeration extension
/// @dev See https://eips.ethereum.org/EIPS/eip-721
///  Note: the VRC-165 identifier for this interface is 0x780e9d63.
interface VRC721Enumerable /* is VRC721 */ {
...
The 0.4.20 Solidity interface grammar is not expressive enough to document the VRC-721 standard. A contract which complies with VRC-721 MUST also abide by the following:
...
- Solidity issue #3419: A contract that implements `VRC721Metadata` or `VRC721Enumerable` SHALL also implement `VRC721`. VRC-721 implements the requirements of interface VRC-165.
...
*If a newer version of Solidity allows the caveats to be expressed in code, then this VIP MAY be updated and the caveats removed, such will be equivalent to the original specification.*Rationale

**NFT Identifiers**

Every NFT is identified by a unique `uint256` ID inside the VRC-721 smart contract. This identifying number SHALL NOT change for the life of the contract. The pair `(contract address, uint256 tokenId)` will then be a globally unique and fully-qualified identifier for a specific asset on an Ethereum chain. While some VRC-721 smart contracts may find it convenient to start with ID 0 and simply increment by one for each new NFT, callers SHALL NOT assume that ID numbers have any specific pattern to them, and MUST treat the ID as a "black box". Also note that a NFTs MAY become invalid (be destroyed). Please see the enumerations functions for a supported enumeration interface.

The choice of `uint256` allows a wide variety of applications because UUIDs and sha3 hashes are directly convertible to `uint256`.

### Transfer Mechanism

VRC-721 standardizes a safe transfer function `safeTransferFrom` (overloaded with and without a `bytes` parameter) and an unsafe function `transferFrom`. Transfers may be initiated by:
...
We chose Standard Interface Detection (VRC-165) to expose the interfaces that a VRC-721 smart contract supports.

A future VIP may create a global registry of interfaces for contracts. We strongly support such an VIP and it would allow your VRC-721 implementation to implement `VRC721Enumerable`, `VRC721Metadata`, or other interfaces by delegating to a separate contract.
...
We remind implementation authors that the empty string is a valid response to `name` and `symbol` if you protest to the usage of this mechanism. We also remind everyone that any smart contract can use the same name and symbol as *your* contract. How a client may determine which VRC-721 smart contracts are well-known (canonical) is outside the scope of this standard.
...
We have adopted `balanceOf`, `totalSupply`, `name` and `symbol` semantics from the VRC-20 specification. An implementation may also include a function `decimals` that returns `uint8(0)` if its goal is to be more compatible with VRC-20 while supporting this standard. However, we find it contrived to require all VRC-721 implementations to support the `decimals` function.
